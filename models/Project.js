...
milestoneSchema.methods.calculateProgress = function() {
    ...
    const totalWeight = this.subMilestones.reduce((sum, sub) => sum + (sub.weight || 0), 0);
    if (totalWeight === 0) return 0;

    const weightedProgress = this.subMilestones.reduce((sum, sub) => {
        return sum + ((sub.progress || 0) * (sub.weight || 0));
    }, 0);
...
// Method to calculate overall project progress based on milestones
projectSchema.methods.calculateOverallProgress = function() {
    if (!this.milestones || this.milestones.length === 0) {
        return this.progress;
    }

    // Calculate total weight
    const totalWeight = this.milestones.reduce((sum, milestone) => sum + (milestone.weight || 0), 0);
    if (totalWeight === 0) return 0;

    // Calculate weighted progress
    const weightedProgress = this.milestones.reduce((sum, milestone) => {
        // First calculate milestone progress from its sub-milestones
        const milestoneProgress = milestone.calculateProgress();
        return sum + (milestoneProgress * (milestone.weight || 0));
    }, 0);

    return Math.round(weightedProgress / totalWeight);
};

// Method to update milestone statuses based on progress
projectSchema.methods.updateMilestoneStatuses = function() {
    this.milestones.forEach(milestone => {
        // Update sub-milestone statuses
        milestone.subMilestones.forEach(subMilestone => {
            ...
            } else if (subMilestone.progress > 0 && subMilestone.progress < 100) {
                subMilestone.status = 'In Progress';
            } else if (subMilestone.dueDate && new Date() > subMilestone.dueDate && subMilestone.progress < 100) {
                subMilestone.status = 'Overdue';
            }
        });
        ...
        } else if (milestone.dueDate && new Date() > milestone.dueDate && milestone.progress < 100) {
    ...
    } else if (this.progress > 0 && this.progress < 100 && ['Planning', 'Approved'].includes(this.status)) {
...
// Pre-save middleware to update progress and statuses
projectSchema.pre('save', function(next) {
    // Update milestone and sub-milestone statuses
