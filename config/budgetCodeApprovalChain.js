...
//  * 3. Finance - final approval and activation
//  */
// const getBudgetCodeApprovalChain = (creatorName, department, budgetCode) => {
//   const chain = [];

//   console.log(`Getting budget code approval chain for: ${creatorName} in ${department}`);
//   console.log(`Budget Code: ${budgetCode.code}, Amount: XAF ${budgetCode.budget}`);

//   // Find the creator in the department structure
//   let creatorData = null;
//   let creatorDepartmentName = department;

//   // Check if creator is a department head
//   if (DEPARTMENT_STRUCTURE[department] && DEPARTMENT_STRUCTURE[department].head === creatorName) {
//     creatorData = {
//       name: creatorName,
//       email: DEPARTMENT_STRUCTURE[department].headEmail,
...
//       assignedDate: new Date(),
//       responsibilities: {
//         validateDepartmentalNeed: true,
//         confirmBudgetAllocation: true,
//         verifyBudgetPeriod: true
//       }
//     });
//   }
...
//         department: 'Executive'
//       },
//       status: executive.headEmail === DEPARTMENT_STRUCTURE[department]?.headEmail ? 'pending' : 'awaiting_previous',
//       assignedDate: new Date(),
//       responsibilities: {
//         strategicAlignment: true,
//         budgetAuthorization: true
...
// /**
//  * Map user roles to their budget code approval authority levels
//  */
// const getBudgetCodeApprovalLevel = (userRole, userEmail) => {
//   // Finance has final authority (Level 3)
//   if (userRole === 'finance') return 3;
...
//   // Supervisor doesn't have budget code approval authority
//   return 0;
// };
...
//   // Check if user role matches the required role for this level
//   const userApprovalLevel = getBudgetCodeApprovalLevel(user.role, user.email);
...
// /**
//  * Validate budget code approval data
//  */
// const validateBudgetCodeApproval = (approvalData) => {
//   const errors = [];

//   if (!approvalData.decision || !['approved', 'rejected'].includes(approvalData.decision)) {
//     errors.push('Valid approval decision is required (approved or rejected)');
//   }

//   if (!approvalData.comments || approvalData.comments.trim().length === 0) {
//     errors.push('Approval comments are required');
//   }

//   if (approvalData.decision === 'rejected' && approvalData.comments.length < 10) {
//     errors.push('Rejection reason must be at least 10 characters');
//   }

//   // Validate budget adjustments if provided
//   if (approvalData.adjustedBudget) {
//     if (typeof approvalData.adjustedBudget !== 'number' || approvalData.adjustedBudget <= 0) {
//       errors.push('Adjusted budget must be a positive number');
//     }
//     if (!approvalData.adjustmentReason) {
//       errors.push('Adjustment reason is required when modifying budget amount');
//     }
//   }
...
//  */
// const processBudgetCodeApproval = async (budgetCode, approvalData, approver) => {
//   // Find current approval step
//   const currentStep = budgetCode.approvalChain.find(
//     step => step.status === 'pending' && step.approver.email === approver.email
//   );
...
//   const validation = validateBudgetCodeApproval(approvalData);
//   if (!validation.isValid) {
//     throw new Error(`Validation failed: ${validation.errors.join(', ')}`);
//   }

//   // Update current step
//   currentStep.status = approvalData.decision === 'approved' ? 'approved' : 'rejected';
//   currentStep.actionDate = new Date();
//   currentStep.actionTime = new Date().toLocaleTimeString();
//   currentStep.comments = approvalData.comments;
//   currentStep.approvedBy = approver.userId;

//   // If approved, activate next step or finalize
//   if (approvalData.decision === 'approved') {
//     const nextStep = budgetCode.approvalChain.find(
//       step => step.level === currentStep.level + 1
//     );

//     if (nextStep) {
//       nextStep.status = 'pending';
//       nextStep.assignedDate = new Date();
...
//       budgetCode.approvedDate = new Date();
//       budgetCode.approvedBy = approver.userId;
//     }
//   } else {
//     // Rejected - set budget code as rejected
//     budgetCode.status = 'rejected';
//     budgetCode.active = false;
//     budgetCode.rejectionReason = approvalData.comments;
//   }

//   // Handle budget adjustments if provided
//   if (approvalData.adjustedBudget && approvalData.decision === 'approved') {
//     const originalBudget = budgetCode.budget;
//     budgetCode.budget = approvalData.adjustedBudget;
    
//     if (!budgetCode.budgetHistory) {
//       budgetCode.budgetHistory = [];
//     }
    
//     budgetCode.budgetHistory.push({
//       previousAmount: originalBudget,
//       newAmount: approvalData.adjustedBudget,
//       reason: approvalData.adjustmentReason,
//       changedBy: approver.userId,
//       changedAt: new Date()
//     });
//   }
