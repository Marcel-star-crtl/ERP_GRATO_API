...
class PDFService {
  ...
  async generateTenderApprovalFormPDF(tender) {
    return new Promise((resolve, reject) => {
      try {
        ...
        // NOTE: This loop uses direct resolveSignaturePath (not renderSignatureImage)
        // because generateTenderApprovalFormPDF is not async-refactored in this patch.
        // To fully support Cloudinary here, convert the Promise wrapper to async.
  ...
  async generateExactPOContent(doc, poData) {
    ...
    // Draw footer on all pages with correct indexing
    const range = doc.bufferedPageRange();
  ...
  generateCashRequestContent(doc, data) {
    ...
    if (isPettyCash) {
      ...
      // Items Table - ONLY if items exist (Petty Cash)
      ...
      // Disbursement History (if multiple disbursements exist)
  ...
  // Purpose Section (Separate from Basic Details)
  ...
  // Get acknowledgment info from latest acknowledged disbursement
  ...
  async drawRequesterAcknowledgmentSignature(doc, yPos, data) {
    ...
    await this.renderSignatureImage(doc, acknowledgment?.signatureLocalPath, centerX + 10, yPos - 28, {
      width: 160,
      height: 36,
      fit: [160, 36]
    });
  ...
  // Requester Details (Cash Request)
  ...
  // Itemized Breakdown (Cash Request)
  ...
  async drawApproverSignatures(doc, yPos, data) {
    ...
    for (let index = 0; index < signatureBlocks.length; index++) {
      ...
      await this.renderSignatureImage(doc, block.step?.decidedBy?.signature, x + 10, lineY - 24, {
        width: 110,
        height: 36,
        fit: [110, 36]
      });
  ...
  // Total Disbursed Summary (Cash Request)
  ...
  // Basic Details (Employee Info Only)
  ...
  drawDisbursementHistory(doc, yPos, data) {
    ...
    if (data.disbursements && data.disbursements.length > 0) {
      ...
      // Check page break before table header
      if (yPos > 700) {
        doc.addPage();
        yPos = 50;
