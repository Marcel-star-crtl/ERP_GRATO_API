...
router.post('/purchase-orders/:poId/approve', 
  ...
  async (req, res) => {
    try {
      ...
      // For approval, signed document is required
      if (decision === 'approved' && !signedDocument) {
        return res.status(400).json({
          success: false,
          message: 'Signed document is required for approval'
        });
      }
      ...
      if (decision === 'approved' && signedDocument) {
        ...
        const fs = require('fs');
        const uploadsDir = path.join(__dirname, '../uploads/signed-documents');
        ...
        const filename = `${po.poNumber}_Level${currentStep.level}_${Date.now()}${path.extname(signedDocument.originalname)}`;
        const finalPath = path.join(uploadsDir, filename);

        console.log('Storing signed document:', {
          tempPath: signedDocument.path,
          finalPath: finalPath,
          originalName: signedDocument.originalname
        });

        // Move file from temp location to signed-documents folder
        if (signedDocument.path && fs.existsSync(signedDocument.path)) {
          fs.copyFileSync(signedDocument.path, finalPath);
          // Delete temp file
          fs.unlinkSync(signedDocument.path);
          console.log('✅ Signed document stored successfully');
        } else {
          console.error('❌ Temp file not found:', signedDocument.path);
...
// Assign PO to department (with signed document) - CRITICAL: This must come before generic :poId routes
...
//         const nextStep = po.approvalChain.find(step => step.level === po.currentApprovalLevel);
//         if (nextStep) {
//           po.status = 'pending_approval'; // Generic pending status
//         }
//       } else if (decision === 'approved' && currentStep.level === maxLevel) {
//         // All approvals complete
//         po.status = 'approved';
...
//         timestamp: new Date(),
//         details: comments || `Purchase order ${decision} by ${currentUser.fullName}`,
...
//       // Get POs that are assigned to departments and pending supervisor approval
//       const pendingPOs = await PurchaseOrder.find({
...
// // Approve PO (Supervisor)
// router.post('/purchase-orders/:poId/supervisor-approve', 
//   authMiddleware, 
//   requireRoles('employee', 'finance', 'admin', 'buyer', 'hr', 'supply_chain', 'technical'), 
//   async (req, res) => {
//     try {
...
//         performedBy: req.user.userId,
//         timestamp: new Date(),
//         details: comments || 'Purchase order approved by supervisor'
//       });
...
//     } catch (error) {
//       console.error('Supervisor approve PO error:', error);
//       res.status(500).json({
//         success: false,
