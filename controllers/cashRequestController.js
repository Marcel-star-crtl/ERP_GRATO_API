...
const submitJustification = async (req, res) => {
  try {
    ...
    // Process documents using Cloudinary storage
    let documents = [];
    if (req.files && req.files.length > 0) {
      console.log(`📎 Processing ${req.files.length} document(s) via Cloudinary...`);
...
const createRequest = async (req, res) => {
  try {
    ...
    // Process attachments using Cloudinary storage
    let attachments = [];
    if (req.files && req.files.length > 0) {
      console.log(`\n📎 Processing ${req.files.length} attachment(s) via Cloudinary...`);
...
/**
 * Process justification decision (supports HR level)
 */
...
const createReimbursementRequest = async (req, res) => {
  try {
    ...
    // Process receipt documents using Cloudinary storage
...
const editCashRequest = async (req, res) => {
  try {
    ...
    let newAttachments = [...request.attachments];
    
    if (req.files && req.files.length > 0) {
      console.log(`Adding ${req.files.length} new attachment(s) via Cloudinary...`);
