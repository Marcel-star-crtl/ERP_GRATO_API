...
//     if (department && department !== 'all') {
//       // If filtering by specific department, check if user has access
//       if (user.role === 'admin' || user.department === department) {
//         query.department = department;
//       } else {
//         // Non-admin users filtering by other departments
//         // Still need to respect access control
//         query = {
//           ...query,
...
//         ...folder.toObject(),
//         userAccess: {
//           canView: true, // They can see it, so they can view
//           canUpload: canUpload,
//           canManage: canManage,
//           canDelete: canDelete
//         }
...
//       }
//     } else {
//       // Non-admin users: show folders based on their access rights
//       query = {
//         $or: [
//           // Public folders
//           { isPublic: true },
//           // Folders in user's department
//           { department: user.department },
//           // Folders that explicitly allow user's department
//           { 'accessControl.allowedDepartments': user.department },
//           // Folders that explicitly allow this specific user
//           { 'accessControl.allowedUsers': req.user.userId },
//           // Folders created by this user
//           { createdBy: req.user.userId }
//         ]
...
//       // If user filters by specific department, add that constraint
//       if (department && department !== 'all') {
//         // Only allow filtering if it's their own department or they have access
...
//           };
//         } else {
//           // For other departments, still respect access control
//           query = {
//             $and: [
//               query,
//               { department: department }
...
//     // Check permission
//     if (!canUserManageFolder(folder, { _id: req.user.userId })) {
//       return res.status(403).json({
//         success: false,
//         message: 'You do not have permission to manage this folder'
//       });
...
//     // Check permission
//     if (!canUserDeleteFolder(folder, { _id: req.user.userId })) {
//       return res.status(403).json({
//         success: false,
//         message: 'You do not have permission to delete this folder'
//       });
...
//     // Check upload permission
//     if (!canUserUploadToFolder(folder, { _id: req.user.userId })) {
//       // Cleanup file
//       if (req.file.path && fs.existsSync(req.file.path)) {
//         fs.unlinkSync(req.file.path);
//       }
...
//       // Check if already shared
//       const existingShare = file.sharedWith.find(s => s.userId.toString() === shareWith);
//       if (!existingShare) {
//         file.sharedWith.push({
//           userId: shareWith,
//           type: shareType || 'view',
...
//       fileId,
//       fileName: file.name,
//       details: { sharedWith, type: shareType }
//     }).save();
...
//   if (folder.department === user.department) return true;
//   if (folder.accessControl?.allowedDepartments?.includes(user.department)) return true;
//   if (folder.accessControl?.allowedUsers?.some(id => id.toString() === user._id.toString())) return true;
//   return false;
// };
