...
const getEvaluationStatistics = async (req, res) => {
  try {
    ...
    if (!['admin', 'supply_chain', 'supervisor'].includes(user.role)) {
      return res.status(403).json({
        success: false,
        message: 'Access denied'
      });
...
// //     // CRITICAL: Verify immediate supervisor relationship (same as behavioral evaluation)
// //     const isDirectReport = supervisor.directReports.some(
// //       report => report._id.equals(employeeId)
// //     );
...
// //     const mongoose = require('mongoose');
// //     const completedTasks = await ActionItem.find({
// //       // assignedTo: mongoose.Types.ObjectId(employeeId),
...
// //         $lte: endDate
// //       },
// //       'completionGrade.score': { $exists: true, $ne: null }
// //     }).populate('linkedKPI.kpiDocId');

// //     console.log(`Found ${completedTasks.length} completed and graded tasks`);
...
// //     for (const kpi of quarterlyKPI.kpis) {
// //       const kpiTasks = completedTasks.filter(task => 
// //         task.linkedKPI && 
// //         task.linkedKPI.kpiDocId && 
// //         task.linkedKPI.kpiDocId.equals(quarterlyKPI._id) &&
// //         task.linkedKPI.kpiTitle === kpi.title
// //       );

// //       if (kpiTasks.length > 0) {
// //         const totalGrade = kpiTasks.reduce((sum, task) => sum + task.completionGrade.score, 0);
// //         const averageGrade = totalGrade / kpiTasks.length;
// //         const achievedPercentage = (averageGrade / 5) * 100;
// //         const weightedScore = (achievedPercentage * kpi.weight) / 100;
...
// //     const averageCompletionGrade = completedTasks.length > 0
// //       ? completedTasks.reduce((sum, task) => sum + task.completionGrade.score, 0) / completedTasks.length
// //       : 0;
...
// //     console.log(`✅ Evaluation generated - Final Score: ${evaluation.finalScore}%`);
...
//     // CRITICAL: Verify immediate supervisor relationship (same as behavioral evaluation)
//     const isDirectReport = supervisor.directReports.some(
//       report => report._id.equals(employeeObjectId)
//     );
...
//     // ✅ Fetch completed tasks for this quarter - use employeeObjectId
//     const completedTasks = await ActionItem.find({
...
//     // ✅ Calculate totals - use employeeObjectId
//     const totalTasks = await ActionItem.countDocuments({
//       assignedTo: employeeObjectId,
//       createdAt: {
//         $gte: startDate,
...
// // Get evaluations (for supervisors/admins) - UPDATED to only show their evaluations
// const getEvaluations = async (req, res) => {
//   try {
//     const userId = req.user.userId;
//     const { quarter, status, employeeId } = req.query;
...
//     if (supervisor.role === 'supervisor' && employee.department !== supervisor.department) {
//       return res.status(403).json({
//         success: false,
...
//     if (!['supervisor', 'admin', 'supply_chain'].includes(supervisor.role)) {
//       return res.status(403).json({
//         success: false,
//         message: 'Only supervisors can generate evaluations'
//       });
//     }
...
//         const totalGrade = kpiTasks.reduce((sum, task) => sum + task.completionGrade.score, 0);
//         const averageGrade = totalGrade / kpiTasks.length;
//         const achievedPercentage = (averageGrade / 5) * 100; // Convert 1-5 scale to percentage
//         const weightedScore = (achievedPercentage * kpi.weight) / 100;
...
//         totalWeightedScore += weightedScore;
//       } else {
//         // No tasks completed for this KPI
//         kpiAchievement.push({
//           kpiTitle: kpi.title,
//           kpiWeight: kpi.weight,
//           tasksCompleted: 0,
...
//     // For supervisors, filter by department
//     if (user.role === 'supervisor') {
//       const departmentEmployees = await User.find({ 
//         department: user.department,
//         role: { $ne: 'supervisor' }
