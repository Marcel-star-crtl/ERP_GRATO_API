...
const { saveFile, deleteFile, STORAGE_CATEGORIES } = require('../utils/cloudinaryStorage');
...
exports.prepareInvoiceFromPO = async (req, res) => {
  try {
    ...
    // Upload file if provided using cloudinaryStorage utility
    let invoiceFileData = null;
    if (req.file) {
      try {
        console.log('📤 Uploading invoice file via cloudinaryStorage...');
...
exports.deleteFinanceInvoice = async (req, res) => {
  try {
    ...
    // Delete from Cloudinary via cloudinaryStorage utility if file exists
    if (invoice.invoiceFile?.publicId) {
      try {
        await deleteFile({ publicId: invoice.invoiceFile.publicId });
      } catch (err) {
