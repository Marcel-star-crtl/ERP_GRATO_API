...
const processSupervisorDecision = async (req, res) => {
  try {
    ...
    if (decision === 'rejected') {
      ...
      const remainingSteps = requisition.approvalChain.filter(step => step.status === 'pending');

      if (remainingSteps.length === 1 && remainingSteps[0]._id.equals(requisition.approvalChain[currentStepIndex]._id)) {
        // This was the last approval step - move to supply chain review
        ...
        if (nextStep) {
          // Check if next step is supply chain related
          if (nextStep.approver.role.includes('Supply Chain') || nextStep.level >= 2) {
            requisition.status = 'pending_supply_chain_review';
          } else {
      ...
      if (decision === 'approved') {
          ...
          if (supplyChainCoordinator) {
              notifications.push(
                  sendEmail({
                      to: supplyChainCoordinator.email,
                      subject: `Purchase Requisition Ready for Supply Chain Review - ${requisition.employee.fullName}`,
                      html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <div style="background-color: #d4edda; padding: 20px; border-radius: 8px; border-left: 4px solid #28a745;">
                <h2 style="color: #155724; margin-top: 0;">Budget Verified - Ready for Supply Chain Review</h2>
                <p>A purchase requisition has been verified by Finance and is ready for supply chain review.</p>
          ...
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <div style="background-color: #d4edda; padding: 20px; border-radius: 8px; border-left: 4px solid #28a745;">
              ...
              <p>Your requisition will now be reviewed by the Supply Chain team for procurement planning and buyer assignment.</p>
...
// NEW: Head of Supply Chain Final Approval
const processHeadApproval = async (req, res) => {
  try {
      const { requisitionId } = req.params;
      const { decision, comments } = req.body;

      console.log('=== HEAD APPROVAL PROCESSING ===');
      console.log('Requisition ID:', requisitionId);
      console.log('Decision:', decision);
      ...
      if (decision === 'approved') {
          ...
          // Reduce buyer workload if rejected
          if (requisition.supplyChainReview?.assignedBuyer) {
              await User.findByIdAndUpdate(requisition.supplyChainReview.assignedBuyer, {
                  $inc: { 'buyerDetails.workload.currentAssignments': -1 }
              });
          }
      }
      ...
      if (decision === 'approved') {
          ...
          if (requisition.supplyChainReview?.assignedBuyer) {
              notifications.push(
                  sendEmail({
                      to: requisition.supplyChainReview.assignedBuyer.email,
            ...
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <div style="background-color: #d4edda; padding: 20px; border-radius: 8px; border-left: 4px solid #28a745;">
                ...
                <p>The purchase requisition assigned to you has been approved and is ready for procurement.</p>
          ...
          notifications.push(
              sendPurchaseRequisitionEmail.approvalToEmployee(
                  requisition.employee.email,
                  requisition.title,
                  requisition._id,
                  user.fullName,
          ...
          notifications.push(
              sendPurchaseRequisitionEmail.denialToEmployee(
                  requisition.employee.email,
                  comments || 'Requisition rejected by Head of Supply Chain',
                  requisition._id,
                  user.fullName
              ).catch(error => ({ error, type: 'employee' }))
          );
      ...
      console.log('=== HEAD APPROVAL COMPLETED ===');
      res.json({
          success: true,
          message: `Requisition ${decision} by Head of Supply Chain`,
          data: requisition,
          notifications: {
              sent: notificationResults.filter(r => r.status === 'fulfilled').length,
              failed: notificationResults.filter(r => r.status === 'rejected').length
...
// NEW: Get Supply Chain Head Requisitions for Final Approval
