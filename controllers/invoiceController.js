...
exports.uploadInvoice = async (req, res) => {
  try {
    ...
    // ✅ Process file uploads using saveFile from cloudinaryStorage
    const uploadedFiles = {};
    const uploadPromises = [];

    if (req.files?.poFile?.length > 0) {
      uploadPromises.push(
        saveFile(req.files.poFile[0], 'supplier-invoices', 'po-files')
          .then(result => {
            uploadedFiles.poFile = result;
            console.log('✅ PO file uploaded:', result.url);
          })
      );
    }

    if (req.files?.invoiceFile?.length > 0) {
      uploadPromises.push(
        saveFile(req.files.invoiceFile[0], 'supplier-invoices', 'invoice-files')
          .then(result => {
            uploadedFiles.invoiceFile = result;
            console.log('✅ Invoice file uploaded:', result.url);
          })
      );
    }
    ...
    if (financeTeam.length > 0) {
      ...
      notifications.push(
        sendEmail({
          ...
          `
        }).catch(err => { console.error('Failed to send finance notification:', err); return { error: err, type: 'finance' }; })
      );
    }
    ...
    if (admins.length > 0) {
      notifications.push(
        sendEmail({
          ...
          `
        }).catch(err => { console.error('Failed to send admin notification:', err); return { error: err, type: 'admin' }; })
      );
    }
    ...
    notifications.push(
      sendEmail({
        ...
        `
      }).catch(err => { console.error('Failed to send employee confirmation:', err); return { error: err, type: 'employee' }; })
    );
    ...
    // ✅ Clean up any files already uploaded to Cloudinary using deleteFile
    if (Object.keys(uploadedFiles ?? {}).length > 0) {
      const cleanupPromises = Object.values(uploadedFiles).map(file => deleteFile(file));
      await Promise.allSettled(cleanupPromises);
      console.log('🗑️  Cleaned up uploaded Cloudinary files');
    }
...
// Process invoice approval/rejection
exports.processInvoiceDecision = async (req, res) => {
  try {
    ...
    const invoice = await Invoice.findById(invoiceId).populate('employee', 'fullName email');
    if (!invoice) {
      return res.status(404).json({ success: false, message: 'Invoice not found' });
    }

    invoice.approvalStatus = decision === 'approved' ? 'approved' : 'rejected';
    if (decision === 'rejected' && comments) invoice.rejectionComments = comments;
    ...
    if (decision === 'approved') {
      try {
        ...
        console.log('✅ Accounting posted for customer invoice');
      } catch (accountingError) {
        console.error('⚠️ Accounting auto-post skipped:', accountingError.message);
      }
    }
    ...
    res.json({ success: true, message: `Invoice ${decision} successfully`, data: invoice });

  } catch (error) {
    console.error('Error processing invoice decision:', error);
    res.status(400).json({ success: false, message: error.message || 'Failed to process decision' });
  }
};
