...
const {
  saveFile,
  STORAGE_CATEGORIES
} = require('../utils/cloudinaryStorage');  // ✅ switched from localFileStorage
...
exports.getSupplyChainPendingPOs = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to fetch pending POs' });
  }
};
...
exports.downloadPOForSigning = async (req, res) => {
  try {
    ...
    const po = await PurchaseOrder.findById(poId).populate('supplierId', 'name email');
    ...
    res.status(500).json({ success: false, message: 'Failed to download PO' });
  }
};
...
exports.assignPOToDepartment = async (req, res) => {
  try {
    ...
    if (po.status !== 'pending_supply_chain_assignment') {
      return res.status(400).json({ success: false, message: 'PO has already been assigned' });
    }
    ...
    // ✅ saveFile now uploads to Cloudinary instead of local disk
    ...
    if (firstApprover) {
      await sendEmail({
        ...
        `
      }).catch(error => console.error('Failed to notify approver:', error));
    }
    ...
    res.status(400).json({ success: false, message: error.message || 'Failed to assign PO' });
  }
};
  ...
  try {
    ...
    if (po.buyerId?.email) {
      await sendEmail({
        ...
        `
      }).catch(error => console.error('Failed to notify buyer:', error));
    }

    res.json({ success: true, message: 'PO rejected successfully', data: po });

  } catch (error) {
    console.error('PO rejection error:', error);
    res.status(400).json({ success: false, message: error.message || 'Failed to reject PO' });
  }
};
