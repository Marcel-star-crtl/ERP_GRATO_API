...
exports.submitApplication = async (req, res) => {
  try {
    ...
    // ✅ Upload files to Cloudinary using saveFile instead of fs.renameSync
    if (req.files) {
    ...
    try {
      ...
      const supplierUser = await User.create({
        ...
        supplierDetails: {
          ...
          documents: exports.normalizeDocuments(applicationData.documents || {}) // ✅ fixed: was `this.normalizeDocuments`
        },
...
exports.updateApplicationStatus = async (req, res) => {
  try {
    ...
    if (status === 'approved') {
      ...
      await exports.createSupplierAccount(application);
    }
    ...
    await exports.sendStatusUpdateEmail(application, oldStatus);
...
exports.uploadAdditionalDocuments = async (req, res) => {
  try {
    ...
    for (const file of req.files.documents) {
      try {
        // ✅ Use saveFile from cloudinaryStorage directly
...
exports.downloadDocument = async (req, res) => {
  try {
    ...
    // Cloudinary URLs are direct HTTPS links — redirect to them
        ...
        if (application) {
          ...
          if (updateData.status) {
            await application.addNote(
              ...
              exports.getNoteType(updateData.status),
              false
            );
          }
...
exports.exportApplications = async (req, res) => {
  try {
    ...
    if (format === 'csv') {
      const csvData = exports.convertToCSV(applications);
  ...
  try {
    ...
    const supplierData = {
      ...
      supplierDetails: {
        ...
        supplierType: exports.mapCategoryToSupplierType(application.category),
