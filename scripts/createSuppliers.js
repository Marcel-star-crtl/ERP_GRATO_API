...
const createAccountsForExistingSuppliers = async () => {
  try {
    ...
    // DEBUG: Let's examine the first supplier in detail
    if (existingSuppliers.length > 0) {
      const firstSupplier = existingSuppliers[0];
      console.log('\n🔍 DEBUG - First supplier data structure:');
      console.log('Name:', firstSupplier.name);
      console.log('Email:', firstSupplier.email);
      console.log('Phone:', firstSupplier.phone);
      console.log('Categories:', firstSupplier.categories);
      console.log('Services:', firstSupplier.services);
      console.log('Address:', firstSupplier.address);
      console.log('Full supplier object keys:', Object.keys(firstSupplier.toObject()));
    }
    ...
    for (const supplier of existingSuppliers) {
      ...
      if (!existingUser) {
        try {
          // Extract contact person name from supplier data or generate one
          const contactPersonName = extractContactPersonName(supplier);
          console.log(`\n📝 Processing ${supplier.name}:`);
          console.log(`   Contact Name: ${contactPersonName}`);
          ...
          console.log(`   Phone: ${supplier.phone || 'NO PHONE'}`);
          
          // Ensure we have all required fields
          const supplierDetails = {
            companyName: supplier.name || 'Unknown Company',
            contactName: contactPersonName || 'Unknown Contact',
            phoneNumber: supplier.phone || '000-000-0000', // Provide default if missing
            supplierType: supplierType || 'General', // Ensure we have a type
            address: {
          ...
          console.log('   Supplier Details to save:', JSON.stringify(supplierDetails, null, 2));

          // Create supplier user account with required supplierDetails
          const supplierUser = await User.create({
            email: supplierEmail,
            password: defaultPassword, // Will be hashed by User model
            ...
            // Required supplierDetails object
            supplierDetails: supplierDetails,

            // Set supplier status based on existing supplier data
            supplierStatus: {
        ...
        if (existingUser.role !== 'supplier') {
          ...
          if (!existingUser.supplierDetails || !existingUser.supplierDetails.companyName) {
            ...
            existingUser.supplierDetails = {
              ...
              phoneNumber: supplier.phone || '000-000-0000',
...
function extractContactPersonName(supplier) {
  ...
  // For well-known companies, let's use more specific names
  if (companyName.toLowerCase().includes('techcorp')) {
    return 'Mr. Emmanuel Mballa';
  } else if (companyName.toLowerCase().includes('office')) {
    return 'Ms. Grace Fotso';
  } else if (companyName.toLowerCase().includes('eco')) {
    return 'Dr. Jean Mvondo';
  }
...
function mapCategoryToSupplierType(categories) {
  ...
  if (!categories || categories.length === 0) {
    console.log('   No categories found, using General');
    return 'General';
  }
  ...
  const mappedType = categoryMap[primaryCategory] || 'General';
  console.log(`   Primary category: ${primaryCategory} -> ${mappedType}`);
  return mappedType;
}
